					   ...
					   message *m_ptr)			/* users pointer to a message */
{
   ...
   */
	...
	if (call_nr < 0 || call_nr > IPCNO_HIGHEST || call_nr >= 32 || !(callname = ipc_call_names[call_nr]))
	{
					  ...
					  size_t size)
{
	...
	if (size == 0)
		return (OK); /* Nothing to do, just return */
   ...
   */
	 ...
	 * a SENDREC.
	 */
		if (r == OK && WILLRECEIVE(caller_ptr->p_endpoint, dst_ptr, (vir_bytes)&table[i].msg, NULL) &&
				   ...
				   struct proc *dst_ptr)
{
	...
	if (size == 0)
		return (EAGAIN);
	if (privp->s_asynendpoint != src_ptr->p_endpoint)
		return EAGAIN;
	if (!may_asynsend_to(src_ptr, proc_nr(dst_ptr)))
		return (ECALLDENIED);
